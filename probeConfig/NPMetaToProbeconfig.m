% =========================================================
% Write out coordinates for a Neuropixels metadata file that works for
% probeConfig
%
% output will be saved in current working directory for matlab
%
% this is based on SGLXMetaToCoords by Jennifer Colonell
%
% output: ch index, x in um, y in um (0), z in um, shank 

 
function NPMetaToProbeconfig
...
if isfield(meta,'snsGeomMap')
    [~, ~, shankPitch, shankInd, xCoord, yCoord, ~] = geomMapToGeom(meta);
elseif isfield(meta,'snsShankMap')    
    [~, ~, shankPitch, shankInd, xCoord, yCoord, ~] = shankMapToGeom(meta);
end
...
for i = 1:nchan
    ...
    fprintf( fid, '%d\t%d\t%d\t%d\t%d\n', i-1, currX, 0, yCoord(i), shankInd(i)+1);
end
fclose(fid);
