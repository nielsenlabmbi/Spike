function MUATrialData(physpath,animal,unit,exp,probeID,eventType,eId,baseTime,stimTime)
...
%document settings
MUAinfo.eventType=eventType;
MUAinfo.eventId=eId;
MUAinfo.baseTime=baseTime;
MUAinfo.stimTime=stimTime;

%copy trial info
MUAinfo.dom=trialInfo.dom;
MUAinfo.domval=trialInfo.domval;
MUAinfo.blankId=trialInfo.blankId;
MUAinfo.triallist=trialInfo.triallist;

%copy position info
MUAinfo.xpos=id.probes(probeID).x;
MUAinfo.zpos=id.probes(probeID).z;
MUAinfo.shaft=id.probes(probeID).shaft;
MUAinfo.zshaft=id.probes(probeID).z+100*(id.probes(probeID).shaft-1);%buffer of 100um between shafts

%save
save([basename '_p' num2str(probeID) '_MUATrial.mat'],'MUA','zMUA','MUAinfo');
